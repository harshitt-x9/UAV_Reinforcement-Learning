...
class BS:
    def __init__(self, x=0, y=0, z=10, N_s=7, N_rf_b=7, N_T=144):
        self.location = np.array([[x, y, z]])
        self.N_s = N_s
        self.N_rf_b = N_rf_b
        self.N_T = N_T

    def calc_f_b(self, mean_AAoD, sep_AAoD, mean_EAoD, sep_EAoD):
        N_rf = 0
        ...
        theta_t_step = (2 * sep_EAoD) / (sample_cluster - 1)
        phi_t_step = (2 * sep_AAoD) / (sample_cluster - 1)

        # Generate the angular ranges for theta_t and phi_t
        theta_t_range = np.arange(-sep_EAoD, sep_EAoD + theta_t_step, theta_t_step)
        phi_t_range = np.arange(-sep_AAoD, sep_AAoD + phi_t_step, phi_t_step)

        # Calculate the final theta_t and phi_t
        theta_t = (mean_EAoD + theta_t_range)[:, np.newaxis]
        phi_t = mean_AAoD + phi_t_range
    ...
    def calc_b_b(self, P_t, K, V_1):
        self.b_b = np.sqrt(P_t/K) * V_1
        return self.b_b 
